...
def make_map():
    """Creates a map of the region

    Returns:
        Basemap.map : Basemap of the region
    """
    m = Basemap(llcrnrlon=-35, llcrnrlat=33, urcrnrlon=35,
                urcrnrlat=61, resolution="i", projection="merc")
    m.drawcoastlines()
    m.drawcountries()
    return m
...
def make_3d_map():
    """Creates a 3D map of the region with the flight levels

    Returns:
        matplotlib.Axes, Basemap.map: Axes and map to further plot trajectories or data
    """
    ax = plt.figure().add_subplot(projection='3d')
    m = Basemap(ax=ax, llcrnrlon=-35, llcrnrlat=33, urcrnrlon=35,
                urcrnrlat=61, resolution="i", projection="merc", fix_aspect=False)
...
def make_atmo_map(FL, time):
    ...
    map = Basemap(llcrnrlon=-30, llcrnrlat=34, urcrnrlon=30,
                  urcrnrlat=60, resolution="i", projection="merc")
    map.drawcoastlines()
    map.drawcountries()
    long_grid, lat_grid = np.meshgrid(long_list, lat_list)
    x, y = map(long_grid, lat_grid)
    cs = map.pcolormesh(x, y, atmo_data[time_index][FL_index],
                        vmin=-0.047278133046295995, vmax=0.34942841580572637)
...
def make_animated_atmo_day_map(FL):
    ...
    # Create map
    fig, ax = plt.subplots()
    m = Basemap(ax=ax, llcrnrlon=-30, llcrnrlat=34, urcrnrlon=30,
                urcrnrlat=60, resolution="i", projection="merc")
    m.drawcoastlines()
    m.drawcountries()
    lat_list, long_list, time_list, fl_list, atmo_data = da.get_atmo_raw_data()
    ...
    def animate_map(time):
        ...
        cs = m.pcolormesh(
            x, y, atmo_data[time][FL_index], vmin=-0.047278133046295995, vmax=0.34942841580572637)
...
def plot_flight_path_on_map_3d_with_atmo_as_points(ax, map, trajectory):
    ...
    for point in flight_path:
        atmo.append(da.get_merged_atmo_data(
...
def plot_flight_path_on_map_3d_with_atmo_as_slices(ax, map, trajectory):
    ...
    for point in flight_path:
        ...
        c1 = da.get_merged_atmo_data(
            xbounds[0], ybounds[0], point[2], point[3])
        c2 = da.get_merged_atmo_data(
            xbounds[0], ybounds[1], point[2], point[3])
        c3 = da.get_merged_atmo_data(
            xbounds[1], ybounds[0], point[2], point[3])
...
def animate_flight_path_on_map(list_of_trajectories, dt):
    ...
    fig, ax = plt.subplots()
    m = Basemap(ax=ax, llcrnrlon=-32, llcrnrlat=32, urcrnrlon=32,
                urcrnrlat=62, resolution="i", projection="merc")
    m.drawcoastlines()
    m.drawcountries()

    def animate_map(time):
        cs = m.scatter(time_grid[time]["LONG"], time_grid[time]["LAT"], c=time_grid[time]
                       ["FL"], cmap="viridis", vmin=100, vmax=400, latlon=True, s=1)
